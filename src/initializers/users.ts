...
export class Users extends Initializer {
  saltRounds: number;
  usersHash: string;
  ...
  async initialize() {
    ...
    api.users.add = async (userName: string, password: string) => {
    ...
    api.users.authenticate = async (
      userName: string,
      password: string
    ): Promise<boolean> => {
      try {
        const dataString = await redis.hget(this.usersHash, userName);
    ...
    api.users.delete = async (userName: string) => {
    ...
    api.users.cryptPassword = async (password: string) => {
      return bcrypt.hash(password, this.saltRounds);
    };

    api.users.comparePassword = async (
